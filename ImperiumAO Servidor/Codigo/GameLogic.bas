       ...
       If UserList(TempCharIndex).flags.AdminInvisible = 0 Or UserList(UserIndex).flags.Privilegios = PlayerType.Dios Then
            ...
            If UserList(TempCharIndex).DescRM = "" Then
                ...
                If Len(UserList(TempCharIndex).Desc) > 1 Then
                   Stat = UserList(TempCharIndex).name & Stat & " - " & UserList(TempCharIndex).Desc & " (" & UserList(TempCharIndex).Clase & " " & UserList(TempCharIndex).Raza & " " & " Nivel " & UserList(TempCharIndex).Stats.ELV & " | "
                Else
                   Stat = UserList(TempCharIndex).name & Stat & " (" & UserList(TempCharIndex).Clase & " " & UserList(TempCharIndex).Raza & " " & " Nivel " & UserList(TempCharIndex).Stats.ELV & " | "
                End If
                ...
                Else
                    If UserList(TempCharIndex).flags.Privilegios > 0 Then
                        Stat = Stat & " <Dungeon Master> ~0~185~0~1~0"
