...
On Error Resume Next
    ...
    arch = App.path & "\Recursos\" & "armas.dat"
...
On Error GoTo errorH:
    ...
    Versiones(1) = Val(GetVar(App.path & "\Recursos\" & "versiones.ini", "Graficos", "Val"))
    Versiones(2) = Val(GetVar(App.path & "\Recursos\" & "versiones.ini", "Wavs", "Val"))
    Versiones(3) = Val(GetVar(App.path & "\Recursos\" & "versiones.ini", "Midis", "Val"))
    Versiones(4) = Val(GetVar(App.path & "\Recursos\" & "versiones.ini", "Init", "Val"))
    Versiones(5) = Val(GetVar(App.path & "\Recursos\" & "versiones.ini", "Mapas", "Val"))
    Versiones(6) = Val(GetVar(App.path & "\Recursos\" & "versiones.ini", "E", "Val"))
    Versiones(7) = Val(GetVar(App.path & "\Recursos\" & "versiones.ini", "O", "Val"))
Exit Sub
...
Sub CargarColores()
    ...
    archivoC = App.path & "\Recursos\colores.dat"
...
On Error Resume Next
    ...
    arch = App.path & "\Recursos\" & "escudos.dat"
...
'**************************************************************
    ...
    If map = 1 Then
       General_Particle_Create 37, 53, 45
       General_Particle_Create 37, 57, 45
       Light.Create_Light_To_Map 53, 45, 1, 255, 255, 255
       Light.Create_Light_To_Map 57, 45, 1, 255, 255, 255
    End If
    
    If map = 2 Then
       General_Particle_Create 37, 51, 21
       Light.Create_Light_To_Map 51, 21, 1, 255, 255, 255
    End If
...
Sub WriteClientVer()
    ...
    hFile = FreeFile()
    Open App.path & "\Recursos\Ver.bin" For Binary Access Write Lock Read As #hFile
    Put #hFile, , CLng(777)
    Put #hFile, , CLng(777)
    Put #hFile, , CLng(777)
...
UserMap = 1
    ...
    'frmPres.Picture = LoadPicture(App.path & "\Recursos\Graficos\bosquefinal.jpg")
...
'**************************************************************
    ...
    fHandle = FreeFile
    Open App.path & "\Recursos\ao.dat" For Binary Access Read Lock Write As fHandle
...
'**************************************************************
    ...
    CityDesc(1) = "En la arena grandes jugadores compiten por ver quien es el mejor."
...
For A = 0 To 9
    Macros(A).comando = GetVar(App.path & "\Recursos\macros.dat", "Macro" & A, "Comando")
    Macros(A).Usar = GetVar(App.path & "\Recursos\macros.dat", "Macro" & A, "Usar")
    Macros(A).Equipar = GetVar(App.path & "\Recursos\macros.dat", "Macro" & A, "Equipar")
    Macros(A).Hechizo = GetVar(App.path & "\Recursos\macros.dat", "Macro" & A, "Hechizo")
Next A
 For A = 0 To 9
...
Sub GuardarMacros()
 Dim A As Byte
For A = 0 To 9
    WriteVar App.path & "\Recursos\macros.dat", "Macro" & A, "Comando", Macros(A).comando
    WriteVar App.path & "\Recursos\macros.dat", "Macro" & A, "Usar", Macros(A).Usar
    WriteVar App.path & "\Recursos\macros.dat", "Macro" & A, "Equipar", Macros(A).Equipar
    WriteVar App.path & "\Recursos\macros.dat", "Macro" & A, "Hechizo", Macros(A).Hechizo
Next A
